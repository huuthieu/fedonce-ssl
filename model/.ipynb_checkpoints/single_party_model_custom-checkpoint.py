...
class SingleParty:
    ...
    def train(self, X, y, X_val, y_val, X_test, y_test):
        ...
        for i in range(self.num_epochs):
            ...
            if X_val is not None and y_val is not None and (i + 1) % self.test_freq == 0:
                self.model.eval()
                with torch.no_grad():
                    if self.task == "binary_classification":
                        y_pred_train, y_score_train = self.predict(X)
                        y_pred_val, y_score_val = self.predict(X_val)
                        ...
                        print("[Final] Epoch {}: train auc {}, val auc {}".format(i + 1, train_auc, val_auc))
                        print("[Final] Epoch {}: best test acc {}, best val f1 {}, best val auc {}, best test f1 {}"
                              .format(i + 1, best_test_acc, best_val_f1, best_val_auc, best_test_f1))
        ...
        return best_test_acc, best_test_f1, best_test_rmse, best_val_auc
