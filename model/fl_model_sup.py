...
from utils.data_utils import get_random_noisy_row
...
class VerticalFLModel:
    ...
    def train_local_party(self, ep, party_id, X, y, model, unalign_index):
        """
        ...
        dataset_combined = LocalDataset(torch.from_numpy(X_combined).float(), y_combined_tensor)
        data_loader_combined = DataLoader(dataset_combined, batch_size=self.local_batch_size, shuffle=True,
                                        drop_last=True, num_workers=self.num_workers,
                                        multiprocessing_context='fork' if self.num_workers > 0 else None)
        ...
        for i in range(self.num_local_rounds):
            start_epoch = datetime.now()
            for j, (idx, X_i, y_i) in enumerate(data_loader_combined, 0):
                X_i = X_i.to(self.device)
                y_i = y_i.to(self.device)
            ...
            if party_id != self.active_party_id:
                ...
                if use_cache and os.path.isfile(local_model_path):
                    ...
                    local_model = self.train_local_party(0, party_id, Xs[party_id],
                                                         y, local_model, unalign_index=unalign_index)
                    pred_labels[party_id, :, :] = self.predict_local(Xs[party_id], local_model, feature=True)
...
#         print("pred_labels[1]: ",pred_labels[1].shape)
    ...
    def predict_local(self, X, model, batch_size=256, feature = False):
        if self.task in ["binary_classification", "regression"]:
            X_tensor = torch.from_numpy(X).float()
