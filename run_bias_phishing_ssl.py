...
#             print(noise_index)
        ...
        print("xs_test[0].shape", xs_test[0].shape) 
        ...
        acc, f1, _, _ = aggregate_model.train(xs_train, y_train, xs_test, y_test, use_cache=False, noise_index = noise_index, ssl = ssl, unalign_index = unalign_index)
    ...
    print("F1 for active party {} with beta {}: {}".format(active_party, beta, str(f1_summary)))
    if overlap_ratio > 0:
        print("F1 for active party {} with overlap ratio {}: {}".format(active_party, overlap_ratio, str(f1_summary)))
...
# results = run_vertical_fl(0.5, True, False, 0, True)
# print("-------------------------------------------------")


# results = run_vertical_fl(0.5, False, False, 0.2)
# print("-------------------------------------------------")


results = run_vertical_fl(beta = 0.5,  ssl = True)
print("-------------------------------------------------")

# overlap_ratios = np.arange(0.1, 0.8, 0.1)
# results = Parallel(n_jobs=6)(delayed(run_vertical_fl)(0.5, False, False, overlap_ratio) for overlap_ratio in overlap_ratios)
# print("-------------------------------------------------")
# for overlap_ratio, (mean, std) in zip(overlap_ratios, results):
#     print("Party {}, overlap_ratio {:.1f}: Accuracy mean={}, std={}".format(0, overlap_ratio, mean, std))
