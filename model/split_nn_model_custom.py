...
class SplitNNModel:
    def __init__(self, num_parties, name="", num_epochs=100, lr=1e-4, weight_decay=1e-4, cls_weight = None,
        ...
        self.agg_model_path = self.model_path + "/agg_model.pth"
        self.local_models_path = []
        for i in range(self.num_parties):
            self.local_models_path.append(self.model_path + "/local_model_{}.pth".format(i))
                ...
                with torch.no_grad():
                    if self.task == 'binary_classification':
                        ...
                        if val_f1 > best_val_f1:
                            best_val_f1 = val_f1
                            ## save best f1 agg model
                            torch.save(model.agg_model.state_dict(), self.agg_model_path)
                            ## save best f1 local model
                            for local_model, local_model_path in zip(model.local_models,self.local_models_path):
                        ...
                        print("[Final] Epoch {}: best val acc {}, best val f1 {}".format(ep + 1, best_val_acc,
                                                                                           best_val_f1))
                        print("[Final] Epoch {}: train auc {}, test auc {}".format(ep + 1, train_auc, val_auc))
    ...
    def eval(self, Xs):
        ## Load agg model
        self.split_nn.agg_model.load_state_dict(torch.load(self.agg_model_path))

        ## Load local model
        for i in range(len(self.local_models_path)):
            self.split_nn.local_models[i].load_state_dict(torch.load(self.local_models_path[i]))
