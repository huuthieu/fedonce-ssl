...
class VerticalFLModel:
    def __init__(self, num_parties, active_party_id, name="", num_epochs=100, num_local_rounds=1, local_lr=1e-4,
                 ...
                 train_order=None, cls_weight = None):
    ...
    def train_aggregation(self, ep, Z, X_active, y, optimizer, scheduler=None):
        ...
        if self.task == "binary_classification":
            if self.cls_weight != None:
                # loss_fn = nn.BCELoss(weight=self.cls_weight)
                loss_fn = nn.BCEWithLogitsLoss(pos_weight=self.cls_weight)
            else:
                loss_fn = nn.BCELoss()
    ...
    def train(self, Xs, y, Xs_test=None, y_test=None, Xs_test_real =None, y_test_real = None, use_cache=True):
        ...
        for party_id in self.train_order:
            if party_id != self.active_party_id:
                ...
                if use_cache and os.path.isfile(local_model_path):
                    ...
                    # # calculate singular value decomposition right singular vectors of Xs[party_id]
                    # svd_value = torch.svd(torch.from_numpy(Xs[party_id]))[0]
                    # pred_labels[party_id, :, :] = svd_value[:, :self.local_output_dim].numpy()
            ...
            if Xs_test is not None and y_test is not None and (ep + 1) % self.test_freq == 0:
                ...
                with torch.no_grad():
                    if self.task == 'binary_classification':
                        ...
                        y_score_test_real = self.predict_agg(Xs_test_real)

                        y_pred_train = np.where(y_score_train > 0.5, 1, 0)
                        y_pred_test = np.where(y_score_test > 0.5, 1, 0)
                        y_pred_test_real = np.where(y_score_test_real > 0.5, 1, 0)
                        print("y_pred_test_real: ", sum(y_pred_test_real))

                        train_acc = accuracy_score(y, y_pred_train)
                        test_acc = accuracy_score(y_test, y_pred_test)
                        train_f1 = f1_score(y, y_pred_train)
                        test_f1 = f1_score(y_test, y_pred_test)
                        test_real_f1 = f1_score(y_test_real, y_pred_test_real)
                        ...
                        if test_real_f1 > best_test_real_f1:
                            best_test_real_f1 = test_real_f1
                        ...
                        print("[Final] Epoch {}: test real f1 {}".format(ep + 1, test_real_f1))
                        print("[Final] Epoch {}: train auc {}, test auc {}".format(ep + 1, train_auc, test_auc))
                        print("[Final] Epoch {}: best test acc {}, best test f1 {}, best test auc {}, best test f1 real {}".format(ep + 1,
                                                                                                             best_test_acc,
                                                                                                             best_test_f1,
                                                                                                             best_test_auc,
                                                                                                             best_test_real_f1))
    ...
    def predict_agg(self, Xs):
        ...
        if self.task == 'binary_classification' and self.cls_weight is not None:
            y_score = self.sigmoid(y_score)
        return y_score
